...
void TSockFactory::sockopen(QString name, QString host, int port)
{
  ...
  if (lastErr == 0) {
    ...
    delete socket; // Errors on opening, delete this shit.
  }
}
...
QString TSockFactory::socklist(QString name_patt, int pos)
{
  ...
  std::cout << "socklist(\"" << name_patt.toStdString().c_str() << "\", " << pos << ");" << std::endl;

  if (pos < 0)
    return ""; // Negative numbers have nothing here to do.
